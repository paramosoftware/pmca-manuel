    ...
    <UICardContainer>
        ...
        <div class="mt-4 md:flex md:flex-row md:justify-between md:items-center">
            ...
            <div class="flex flex-row items-center justify-between mt-5 md:mt-0">
                <UIIcon 
                    ...
                    @click="listStore.sortByName()" v-if="total > 1" 
                />

                <UPagination v-model="page" :total="total" :page-count="pageSize" show-last show-first :max="6"  size="sm" v-if="total > pageSize" />
            </div>
        </div>
        ...
        <div class="mt-5 flex justify-end">
            <div v-if="total > pageSize">
                <UPagination v-model="page" :total="total" :page-count="pageSize" show-last show-first :max="6" size="sm" />
            </div>
        </div>

        <ModalDialog v-model="isModalDialogOpen" :title="modalTitle" :message="modalMessage"
            :confirmButtonText="modalButtonText" @confirm="deleteItem()" @close="isModalDialogOpen = false" />
