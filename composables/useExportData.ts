export default function useExportData() {
    ...
    async function download(format: DataTransferFormat, addMedia: boolean = false, where?: Where) {
        ...
        const response = await fetch('/api/entry/export?format=' + format + '&addMedia=' + addMedia + (where ? ('&where=' + JSON.stringify(where)) : ''));
