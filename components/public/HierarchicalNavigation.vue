:
<template>
    <div id="containerPointer" class="flex h-full max-w-full w-auto">
        <aside
            id="tree-navigation"
            :class="
                closeClass +
                ' hover:' +
                openClass +
                'rounded-tl-lg rounded-bl-lg shadow-lg border w-full bg-gray-200 border-gray-300 transition-all duration-300 max-h-screen  overflow-auto'
            "
            v-if="1 + 1"
            ref="treeNavigationRef"
        >
            <div class="flex py-4 pl-4 pr-8 h-full">
...
const isTreeNavigationPinned = ref(true);
...
onMounted(() => {
    ...
    window.addEventListener('resize', async () => {
        ...
        if (!isSmallScreen.value) {
            await nextTick();
            isTreeNavigationOpen.value =
                isTreeNavigationOpen.value || isTreeNavigationPinned.value;
